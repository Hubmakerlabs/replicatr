...
func (E *T) Unmarshal(buf *text.Buffer) (err error) {
	...
	var which byte
	// T can have one or no subscription IDs, if it is present we want to collect it
	// before looking for the filters.
	...
	// Next, find the comma (there must be one and at least one object brace after
	// it
	if err = buf.ScanThrough(','); err != nil {
		return
	}
