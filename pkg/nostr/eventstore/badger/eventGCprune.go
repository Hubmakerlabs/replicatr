...
// PruneL1Only implements a simple prune that deletes an event and all related
// indexes.
func PruneL1Only(b *Backend, serials del.Items) (err error) {
...
// PruneHasL2 implements a prune where only events and access records are
// deleted. This way searches can find them and fetch them from an L2 event
// store.
func PruneHasL2(b *Backend, serials del.Items) (err error) {
	err = b.DB.Update(func(txn *badger2.Txn) (err error) {
...
// EventGCPrune implements the EventGCPrune function. If hasL2 is true, a separate prune
// function is called.
func EventGCPrune(hasL2 bool) func(bi any, serials del.Items) (err error) {
	return func(bi any, serials del.Items) (err error) {
		...
		if !hasL2 {
			err = PruneL1Only(b, serials)
			chk.E(err)
			return
		}
		err = PruneHasL2(b, serials)
		chk.E(err)
		return
	}
}
