...
func (b *Backend) QueryEvents(c context.T, f *filter.T) (ch event.C, err error) {
	...
	go b.AccessLoop(c, accessChan, f.ToObject().String())
	...
	go func() {
		defer func() {
			...
			close(accessChan)
			log.T.Ln("query goroutine terminated")
		}()
		// actually iterate
		for _, q1 := range queries {
			log.T.Ln("query", q1.queryFilter.ToObject().String())
			select {
			case <-c.Done():
			...
			go func() {
				...
				if err != nil {
					...
					log.I.Ln("query results channel clear", q2.queryFilter.ToObject().String())
					return
				}
				...
				// q2.results <- Results{}
				log.T.Ln("closing results channel", q2.queryFilter.ToObject().String(), b.Path)
				close(q2.results)
				log.T.Ln("draining results channel")
				for res := range q2.results {
					log.W.Ln("stuck result", res.Ev.ToObject().String())
				}
				log.T.Ln("results channel clear",
					q2.queryFilter.ToObject().String())
			}()
		}
