...
func (b *Backend) QueryEvents(c context.T, f *filter.T) (ch event.C, err error) {
	...
	// Events should not be duplicated in the return to the client, so a mutex and
	// eventid.T map will indicate if an event has already been returned.
	var evMx sync.Mutex
	events := make(map[eventid.T]struct{})
		...
		for {
			...
			case ev1 := <-ch1:
				...
				if ev1.Sig == "" && ev1.ID != "" {
					...
					go func() {
						ch3, err3 := b.L2.QueryEvents(c, &filter.T{IDs: tag.T{ev1.ID.String()}})
						chk.E(err3)
					out2:
						for {
							select {
