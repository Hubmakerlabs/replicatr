...
func (f *T) Duplicate() (dup *T) {
	dup = &T{
		IDs:     make(tag.T, 0, len(f.IDs)),
		Kinds:   make(kinds.T, 0, len(f.Kinds)),
		Authors: make(tag.T, 0, len(f.Authors)),
		Tags:    make(TagMap, len(f.Tags)),
		Search:  f.Search,
	}
	if f.Since != nil {
		dup.Since = &(*f.Since)
	...
	for _, v := range f.IDs {
		dup.IDs = append(dup.IDs, v)
	}
	for _, v := range f.Kinds {
		dup.Kinds = append(dup.Kinds, v)
	}
	for _, v := range f.Authors {
		dup.Authors = append(dup.Authors, v)
	}
...
func (f *T) ToObject() (o object.T) {
	...
	if len(f.IDs) > 0 {
		o = append(o, object.KV{Key: "ids", Value: f.IDs})
	}
	if len(f.Kinds) > 0 {
		o = append(o, object.KV{Key: "kinds", Value: f.Kinds.ToArray()})
	}
	if len(f.Authors) > 0 {
		o = append(o, object.KV{Key: "authors", Value: f.Authors})
	}
