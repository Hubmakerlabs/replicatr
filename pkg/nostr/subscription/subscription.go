...
import (
	...
	"mleku.dev/git/nostr/interfaces/relay"
...
func (sub *T) DispatchEvent(evt *event.T) {
	...
	go func() {
		...
		defer sub.mu.Unlock()
		if sub.live.Load() {
			log.I.Ln("waiting to dispatch event")
			select {
			case sub.Events <- evt:
				log.I.Ln("sent event to events chan")
			case <-sub.Context.Done():
			}
			log.I.Ln("dispatched event")
		}
		if added {
			log.I.Ln("subscription wait group done")
			sub.storedwg.Done()
		}
	}()
}
...
func (sub *T) DispatchEose() {
	...
	if sub.eosed.CompareAndSwap(false, true) {
		go func() {
			log.I.Ln("wait on wg for sub")
			sub.storedwg.Wait()
			log.I.Ln("signal EOSE")
			sub.EndOfStoredEvents <- struct{}{}
			log.I.Ln("EOSE signal sent")
		}()
	}
}
