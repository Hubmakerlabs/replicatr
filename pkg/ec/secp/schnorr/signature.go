...
var (
	...
	// TODO: change to BIP 340 version
...
func schnorrVerify(sig *Signature, hash []byte,
	...
	// 3. Fail if r >= p
	// 4. Fail if s >= n
	// 5. e = SHA-256(r || m) (Ensure r is padded to 32 bytes)
	// 6. Fail if e >= n
	// 7. R = s*G + e*Q
	...
	// Step 5.
	//
	// e = SHA-256(r || m) (Ensure r is padded to 32 bytes)
...
// schnorrVerifyBlake256 attempt to verify the signature for the provided hash and
// secp256k1 public key and either returns nil if successful or a specific error
// indicating why it failed if not successful.
//
// This differs from the exported Verify method in that it returns a specific
...
// VerifyBlake256 returns whether the signature is valid for the provided hash
// and secp256k1 public key.
func (sig *Signature) VerifyBlake256(hash []byte,
	pubKey *secp256k1.PublicKey) bool {
	return schnorrVerifyBlake256(sig, hash, pubKey) == nil
}
...
// message) using the given nonce and secret key.  The produced signature is
...
func schnorrSignBlake256(secKey, nonce *secp256k1.ModNScalar,
	hash []byte) (*Signature,
	error) {
	// The algorithm for producing a EC-Schnorr-DCRv0 signature is described in
	// README.md and is reproduced here for reference:
...
// NOT be 0.  Since this is an internal use function, these preconditions MUST
// be satisified by the caller.
func schnorrSign(secKey, nonce *secp256k1.ModNScalar, hash []byte) (*Signature,
	error) {
	// The algorithm for producing a EC-Schnorr-DCRv0 signature is described in
	// README.md and is reproduced here for reference:
	//
	...
	// 5. Negate nonce k if R.y is odd (R.y is the y coordinate of the point R)
	// 6. r = R.x (R.x is the x coordinate of the point R)
	// 7. e = SHA-256(r || m) (Ensure r is padded to 32 bytes)
	// 8. Repeat from step 3 (with iteration + 1) if e >= n
	// 9. s = k - e*d mod n
	// 10. Return (r, s)
...
// the given secret key.  The produced signature is deterministic (same message
...
func Sign(secKey *secp256k1.SecretKey, hash []byte) (*Signature, error) {
	// The algorithm for producing a EC-Schnorr-DCRv0 signature is described in
	// README.md and is reproduced here for reference:
	//
	// G = curve generator
	...
	for iteration := uint32(0); ; iteration++ {
		...
		// parameterized by the secret key, message being signed, extra data
		// that identifies the scheme, and an iteration count
		k := secp256k1.NonceRFC6979(secKeyBytes[:], hash,
			rfc6979ExtraDataV0[:],
			nil, iteration)
