...
func (rl *Relay) handleRequest(ctx context.Context, id nip1.SubscriptionID, eose *sync.WaitGroup, ws *WebSocket, filter nip1.Filter) error {
	defer eose.Done()
	...
	for _, reject := range rl.RejectFilter {
		if reject, msg := reject(ctx, filter); reject {
			ws.WriteJSON(nip1.NoticeEnvelope{Text: msg})
	...
	for _, query := range rl.QueryEvents {
		...
		if err != nil {
			ws.WriteJSON(nip1.NoticeEnvelope{Text: err.Error()})
			eose.Done()
			continue
		}
		...
		go func(ch chan *nip1.Event) {
			for event := range ch {
				...
				ws.WriteJSON(nip1.EventEnvelope{SubscriptionID: id, Event: event})
			}
			eose.Done()
		}(ch)
	}
...
func (rl *Relay) handleCountRequest(ctx context.Context, ws *WebSocket, filter nip1.Filter) int64 {
	...
	for _, count := range rl.CountEvents {
		...
		if err != nil {
			ws.WriteJSON(nip1.NoticeEnvelope{err.Error()})
		}
		subtotal += res
	}
